...
class Assinatura_Model_DbTable_TbDocumentoAssinatura extends MinC_Db_Table_Abstract
{
    ...
    public function obterDocumentosDevolvidos($where = [], $order = ['idDocumentoAssinatura DESC'], $inicio = -1, $tamanho = 100, $search = '')
    {
        ...
        $query->from(
            ...
            [
                'idDocumentoAssinatura',
                'dias' => 'DATEDIFF(DAY, dt_criacao, GETDATE())',
            ],
            $this->_schema
        );
        ...
        if (!empty($search) && strlen($search) > 4) {
            $query->where("AnoProjeto+Sequencial LIKE '$search%' OR nomeProjeto LIKE '%$search%' OR Verificacao.Descricao = ?", $search);
        }
    ...
    public function obterTotalDocumentosDevolvidos($where = [])
    {
        ...
        $query->from(
            ["tbDocumentoAssinatura" => $this->_name],
            [
                new Zend_Db_Expr('count(idDocumentoAssinatura)'),
            ],
            $this->_schema
        );
